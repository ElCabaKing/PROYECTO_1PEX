...
export async function appGetRapairList() {
    const res = await axios.get(`${API_URL}/api/repair/getRepairList`);
...
export async function appUpdateHead(repair_id, new_status) {
    try {
        const res = await axios.patch(`${API_URL}/api/repair/updateHead`,
            {
                new_status,
                repair_id
            },
...
export async function appSaveRepair(cedula_cliente, modelo, repair_problem) {
    try {
        await axios.post(`${API_URL}/api/repair/saveRepair`,
            {
                cedula_cliente,
                modelo,
                repair_problem
...
export async function appGetUserRepairList() {
    try {
        const list = await axios.get(`${API_URL}/api/repair/getUsersRepair`,
            {
                withCredentials: true
            }
        );
